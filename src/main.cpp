...
void initCmds(CmdMgr *mgr) {
  ...
  mgr->regCmd(testCmd);
  Cmd *readGraphCmd = new ReadGraphCmd("read_graph");
  mgr->regCmd(readGraphCmd);
  Cmd *writeTreeBFSCmd = new WriteTreeBFSCmd("write_tree_bfs");
  mgr->regCmd(writeTreeBFSCmd);
  Cmd *writeTreeDFSCmd = new WriteTreeDFSCmd("write_tree_dfs");
  mgr->regCmd(writeTreeDFSCmd);
  Cmd *dijkstraCmd = new DijkstraCmd("write_dijkstra");
  mgr->regCmd(dijkstraCmd);
}
